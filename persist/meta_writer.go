...
func (w *MetaTableWriter) Insert(tableName string, data map[string]interface{}) (interface{}, error) {
	...
	for name, v := range subData {
		...
		if err := w.insertSubTable(meta, f, v, pk); err != nil {
			return nil, err
		}
	}
	return pk, nil
...
func (w *MetaTableWriter) insertSubTable(parentMeta *TableMeta, f *FieldMeta, v interface{}, parentPk interface{}) error {
	if parentPk == nil {
	...
	if f.StorageType == StorageOne2One {
		if m, ok := v.(map[string]interface{}); ok {
			return w.insertSubRow(f, m, fk, parentPk)
		}
		return nil
	}
	// ONE2MANY
	if items, ok := v.([]interface{}); ok {
		for _, item := range items {
			if m, ok := item.(map[string]interface{}); ok {
				if err := w.insertSubRow(f, m, fk, parentPk); err != nil {
					return err
				}
			}
		}
...
func (w *MetaTableWriter) insertSubRow(f *FieldMeta, subData map[string]interface{}, fk string, parentPk interface{}) error {
